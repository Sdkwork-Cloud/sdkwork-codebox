...
import { syncCurrentProvidersLiveSafe } from "@/lib/api/postChangeSync";
...
export function useSettings(): UseSettingsResult {
  ...
  // 即时保存设置（用于通用/外观/运行偏好等实时更新场景）
  // 保存基础配置，并同步需要立即生效的桌面级系统行为。
  const autoSaveSettings = useCallback(
    async (updates: Partial<SettingsFormState>): Promise<SaveResult | null> => {
      const previousSettings = settings ?? data ?? null;
      const mergedSettings = previousSettings
      ...
      try {
        ...
        const { webdavSync: _ignoredWebdavSync, ...restSettings } =
          mergedSettings;
        const windowBehavior = normalizeWindowBehavior(restSettings);
        ...
        if (
          ...
          payload.launchOnStartup !== previousSettings?.launchOnStartup
        ) {
          try {
        ...
        if (
          ...
          nextSkipClaudeOnboarding !==
            (previousSettings?.skipClaudeOnboarding ?? false)
        ) {
          try {
        ...
        if (
          ...
          payload.enableClaudePluginIntegration !==
            previousSettings?.enableClaudePluginIntegration
        ) {
          try {
        ...
        if (payload.showInTray !== (previousSettings?.showInTray ?? true)) {
          try {
            await settingsApi.setTrayVisibility(payload.showInTray);
          } catch (error) {
            console.warn("[useSettings] Failed to sync tray visibility", error);
            toast.error(
              t("notifications.syncTrayVisibilityFailed", {
                defaultValue: "同步系统托盘状态失败",
              }),
            );
          }
        }
  ...
  const saveSettings = useCallback(
    async (
      ...
      try {
        ...
        if (payload.showInTray !== (data?.showInTray ?? true)) {
          try {
            await settingsApi.setTrayVisibility(payload.showInTray);
          } catch (error) {
            console.warn("[useSettings] Failed to sync tray visibility", error);
