...
All notable changes to CodeBox will be documented in this file.
...
- **OpenClaw Integration**: Full management support for OpenClaw as the fifth application in CodeBox, including provider switching, configuration panels (Env / Tools / Agents Defaults), Workspace file management (HEARTBEAT / BOOTSTRAP / BOOT), daily memory files, and additive overlay mode
...
- **Crash Logging** - Panic hook captures crash info to `~/.codebox/crash.log` with full stack traces (#562)
...
- **Windows Title** - Display "CodeBox" instead of default "Tauri app" in window title
...
- **Import/Export** - Restrict SQL import to CodeBox exported backups only; refresh providers immediately after import
...
- **ArchLinux installation support** (#259) - Added AUR installation via `paru -S codebox-bin`
...
- **Deep link support** - Import Gemini providers via `codebox://` protocol
...
#### Deep Link Protocol (codebox://)

- **Protocol registration** - `codebox://` URL scheme for one-click imports
...
- **Standardized release artifact naming** - All platform releases now use consistent version-tagged filenames:
  - macOS: `CodeBox-v{version}-macOS.tar.gz` / `.zip`
  - Windows: `CodeBox-v{version}-Windows.msi` / `-Portable.zip`
  - Linux: `CodeBox-v{version}-Linux.AppImage` / `.deb`
...
#### Backup on v1→v2 Migration (codebox internal config)

- When the app detects an old v1 config structure at `~/.codebox/config.json`, it now creates a timestamped backup before writing the new v2 structure.
- Backup location: `~/.codebox/config.v1.backup.<timestamp>.json`
- This only concerns codebox's own metadata file; your actual provider files under `~/.claude/` and `~/.codex/` are untouched.
