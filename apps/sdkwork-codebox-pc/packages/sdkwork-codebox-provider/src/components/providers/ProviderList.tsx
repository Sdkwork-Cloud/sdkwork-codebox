...
export function ProviderList({
    ...
    onSuccess: (imported) => {
      ...
      if (isAdditiveApp) {
        toast.info(
          t(
            hasProviders
              ? "provider.importNothingNew"
              : "provider.importNothingFound",
          ),
        );
        return;
      }

      toast.info(t("provider.importOnlyWhenEmpty"));
    },
    onError: (error: Error) => {
  ...
  return (
    <div className="mt-4 flex min-h-full flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-[24px] border border-border/65 bg-[linear-gradient(180deg,hsl(var(--surface-2)/0.9)_0%,hsl(var(--surface-3)/0.8)_100%)] px-4 py-3 shadow-[0_18px_42px_-36px_hsl(var(--shadow-color)/0.88)]">
        <div className="min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-semibold text-foreground">
              {t("provider.listSummary", {
                count: sortedProviders.length,
                defaultValue: `${sortedProviders.length} 个供应商`,
              })}
            </span>
            {searchTerm.trim() ? (
              <span className="inline-flex items-center rounded-full border border-primary/20 bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">
                {t("provider.searchResultCount", {
                  ...
                  defaultValue: `${filteredProviders.length} 条匹配`,
                })}
              </span>
            ) : null}
          </div>
          <p className="text-xs leading-5 text-muted-foreground">
            {appId === "openclaw" || appId === "opencode"
              ? t("provider.additiveModeHint", {
                  defaultValue: "支持从当前 live 配置导入，并以累加模式维护启用状态。",
                })
              : t("provider.listHint", {
                  defaultValue:
                    "拖拽调整顺序，搜索快速定位供应商，并随时导入当前实时配置。",
                })}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            className={cn(
              "rounded-2xl px-4",
              !isSearchOpen && "border-border/70 bg-background/80",
            )}
            aria-label={t("provider.searchAction", {
              defaultValue: "搜索供应商",
            })}
          >
            <Search className="h-4 w-4" />
            {t("provider.searchAction", {
              defaultValue: "搜索供应商",
            })}
          </Button>
          ...
          {isAdditiveApp ? (
            <Button
              ...
              className="rounded-2xl border-border/70 bg-background/80 px-4"
              aria-label={t("provider.importCurrent", {
                defaultValue: "导入当前配置",
              })}
            >
      ...
      {filteredProviders.length === 0 ? (
        <div className="flex min-h-[260px] flex-1 items-center justify-center rounded-[24px] border border-dashed border-border/80 bg-background/55 px-6 py-8 text-sm text-center text-muted-foreground">
          <div className="max-w-md space-y-2">
            <p className="text-base font-medium text-foreground">
              {t("provider.noSearchResultsTitle", {
