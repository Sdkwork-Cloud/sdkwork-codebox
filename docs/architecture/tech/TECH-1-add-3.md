> Migrated from `docs/user-manual/zh/2-providers/2.1-add.md` on 2026-06-24.
...
> 💡 认证类型由 CodeBox 自动检测（PackyCode API 代理 / Google OAuth / 通用 API Key），无需手动配置。
...
1. 点击或访问深度链接
2. CodeBox 自动打开并显示导入确认
3. 预览配置信息
4. 点击「确认导入」

**获取深度链接**：
- 从他人分享获取
- 使用 [在线生成工具](https://farion1231.github.io/codebox/deplink.html) 创建
...
> ⚠️ **注意**：导入会覆盖现有数据库，建议先导出当前配置作为备份。导出的文件名格式为 `codebox-export-{时间戳}.sql`。
