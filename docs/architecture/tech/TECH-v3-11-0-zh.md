> Migrated from `docs/release-notes/v3.11.0-zh.md` on 2026-06-24.
...
CodeBox v3.11.0 是一次大规模更新，新增第五个应用 **OpenClaw** 的完整管理支持，同时带来全新的**会话管理器**和**备份管理**功能。此外，**Oh My OpenCode (OMO) 集成**、供应商切换的**部分键值合并**架构升级、**设置页面重构**等多项改进使整体体验更加完善。
...
CodeBox 新增对 OpenClaw 的完整管理支持，这是继 Claude Code、Codex、Gemini CLI、OpenCode 之后的第五个受管理应用。
...
访问 [Releases](https://github.com/farion1231/codebox/releases/latest) 下载对应版本。
...
| 文件                                     | 说明                                |
| ---------------------------------------- | ----------------------------------- |
| `CodeBox-v3.11.0-Windows.msi`          | **推荐** - MSI 安装包，支持自动更新 |
| `CodeBox-v3.11.0-Windows-Portable.zip` | 便携版，解压即用，不写入注册表      |

### macOS

| 文件                             | 说明                                                      |
| -------------------------------- | --------------------------------------------------------- |
| `CodeBox-v3.11.0-macOS.zip`    | **推荐** - 解压后拖入 Applications 即可，Universal Binary |
| `CodeBox-v3.11.0-macOS.tar.gz` | 用于 Homebrew 安装和自动更新                              |
...
| 发行版                                  | 推荐格式    | 安装方式                                                               |
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i CodeBox-*.deb` 或 `sudo apt install ./CodeBox-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i CodeBox-*.rpm` 或 `sudo dnf install ./CodeBox-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./CodeBox-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | 添加执行权限后直接运行，或使用 AUR                                     |
| 其他发行版 / 不确定                     | `.AppImage` | `chmod +x CodeBox-*.AppImage && ./CodeBox-*.AppImage`              |
