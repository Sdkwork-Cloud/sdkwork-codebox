> Migrated from `docs/user-manual/ja/5-faq/5.1-config-files.md` on 2026-06-24.
...
`<default-local-config-dir>/settings.json` にはデバイス単位の設定が保存され、常にローカルに保持されます。

### データベースの内容

`codebox.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（CodeBox の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
CodeBox が設定を変更する際の優先順位：

1. **CodeBox データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（CodeBox がバックフィルする）
- CodeBox の `settings.json`

### 手動編集を推奨しないもの

- `codebox.db` データベースファイル
- バックアップファイル
...
CodeBox v3.7.0 で JSON ファイルから SQLite に移行しました：
