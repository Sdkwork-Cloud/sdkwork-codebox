> Migrated from `docs/user-manual/en/5-faq/5.1-config-files.md` on 2026-06-24.
...
- All platforms: `~/.sdkwork/codebox/`
- Windows expanded path: `%USERPROFILE%\\.sdkwork\\codebox\\`
...
`<default-local-config-dir>/settings.json` stores device-level settings and always stays on the local machine.

### Database Contents

`codebox.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to CodeBox's "providers") |
...
CodeBox's priority when modifying configurations:

1. **CodeBox Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by CodeBox)
- CodeBox's `settings.json`

### Not Recommended to Edit Manually

- `codebox.db` database file
- Backup files
...
CodeBox v3.7.0 migrated from JSON files to SQLite:
