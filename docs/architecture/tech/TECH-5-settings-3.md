> Migrated from `docs/user-manual/zh/1-getting-started/1.5-settings.md` on 2026-06-24.
> Owner: SDKWork maintainers

本节介绍如何根据个人偏好配置 CodeBox。
...
开启后，CodeBox 在切换供应商时会自动同步配置到 VS Code 中的 Claude Code 插件（写入 `~/.claude/config.json` 的 `primaryApiKey`）。
...
CodeBox 自身数据的存储位置，默认统一为 `~/.sdkwork/codebox/`；在 Windows 上会展开为 `%USERPROFILE%\\.sdkwork\\codebox\\`。
...
配置 CodeBox 的出站 HTTP/HTTPS 代理，适用于需要通过代理访问外部 API 的场景。
