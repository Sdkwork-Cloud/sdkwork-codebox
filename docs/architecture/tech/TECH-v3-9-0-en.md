> Migrated from `docs/release-notes/v3.9.0-en.md` on 2026-06-24.
...
CodeBox v3.9.0 is the stable release of the v3.9 beta series (`3.9.0-1`, `3.9.0-2`, `3.9.0-3`).
...
- SQL import is restricted to CodeBox exported backups to reduce the risk of importing unsafe or incompatible SQL dumps.
- Proxy takeover modifies CLI live configs; CodeBox will back up the live config before redirecting it to the local proxy. If you want to revert, disable takeover/stop the proxy and restore from the backup when needed.
...
Visit [Releases](https://github.com/farion1231/codebox/releases/latest) to download the appropriate version.
...
| File                                    | Description                                        |
| --------------------------------------- | -------------------------------------------------- |
| `CodeBox-v3.9.0-Windows.msi`          | **Recommended** - MSI installer with auto-update support |
| `CodeBox-v3.9.0-Windows-Portable.zip` | Portable version, no installation required         |

### macOS

| File                            | Description                                                       |
| ------------------------------- | ----------------------------------------------------------------- |
| `CodeBox-v3.9.0-macOS.zip`    | **Recommended** - Extract and drag to Applications, Universal Binary |
| `CodeBox-v3.9.0-macOS.tar.gz` | For Homebrew installation and auto-update                         |
...
| --------------------------------------- | ------------------ | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`             | `sudo dpkg -i CodeBox-*.deb` or `sudo apt install ./CodeBox-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`             | `sudo rpm -i CodeBox-*.rpm` or `sudo dnf install ./CodeBox-*.rpm`  |
| openSUSE                                | `.rpm`             | `sudo zypper install ./CodeBox-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage`        | Make executable and run directly, or use AUR                           |
| Other distros / Unsure                  | `.AppImage`        | `chmod +x CodeBox-*.AppImage && ./CodeBox-*.AppImage`              |
| Sandboxed installation                  | `.flatpak`         | `flatpak install CodeBox-*.flatpak`                                  |
