> Migrated from `docs/user-manual/ja/3-extensions/3.1-mcp.md` on 2026-06-24.
...
あるアプリのスイッチをオンにすると、CodeBox は以下を実行します：
...
あるアプリのスイッチをオフにすると、CodeBox は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、CodeBox にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode）
3. CodeBox が既存の設定を読み取ってインポート
