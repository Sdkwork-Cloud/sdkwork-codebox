> Migrated from `docs/user-manual/zh/5-faq/5.1-config-files.md` on 2026-06-24.
...
- 所有平台：`~/.sdkwork/codebox/`
- Windows 展开路径：`%USERPROFILE%\\.sdkwork\\codebox\\`
...
`<default-local-config-dir>/settings.json` 用于保存设备级设置，并始终保留在本机。

### 数据库内容

`codebox.db` 是 SQLite 数据库，存储：
...
| 字段 | 说明 |
|------|------|
| `models.providers` | 供应商配置（映射为 CodeBox 的"供应商"） |
| `env` | 环境变量配置 |
...
- CLI 工具的配置文件（会被 CodeBox 回填）
- CodeBox 的 `settings.json`
...
CodeBox v3.7.0 从 JSON 文件迁移到 SQLite：
