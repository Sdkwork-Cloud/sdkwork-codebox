> Migrated from `docs/user-manual/en/5-faq/5.4-env-conflict.md` on 2026-06-24.
> Owner: SDKWork maintainers

## Overview

CodeBox automatically detects conflicts between system environment variables and app configurations, preventing configurations from being unexpectedly overridden.
...
Found X environment variables that may conflict with CodeBox configuration
[Expand] [Dismiss]
```
...
3. CodeBox will automatically back up and remove the selected variables

### Automatic Backup

A backup is automatically created before removal:

- Backup location: `<default-local-config-dir>/backups/`
...
If you prefer not to use CodeBox to remove variables, you can handle them manually:
...
- CodeBox provider configurations being overridden
- API requests being sent to the wrong endpoint
- Using the wrong API key

## Best Practices

1. **Use CodeBox to manage configurations**: Avoid setting API keys in system environment variables
...
1. Find the backup file in `<default-local-config-dir>/backups/`
