> Migrated from `docs/user-manual/en/5-faq/5.2-questions.md` on 2026-06-24.
...
2. Add CodeBox to your antivirus software's whitelist
...
chmod +x CodeBox-*.AppImage

# If it still fails, try
./CodeBox-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the active CodeBox data directory exists
2. Restore from backup: `<app-config-dir>/backups/`
...
**Solutions**:
1. Confirm the file was exported by CodeBox
...
**Windows**:
- Check taskbar settings to ensure the CodeBox icon is not hidden
...
2. Restart the app
3. Delete `<default-local-config-dir>/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask codebox`
...
1. Visit [GitHub Issues](https://github.com/farion1231/codebox/issues)
