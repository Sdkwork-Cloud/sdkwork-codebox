> Migrated from `docs/user-manual/en/4-proxy/4.2-takeover.md` on 2026-06-24.
> Owner: SDKWork maintainers

## Overview

App takeover means letting CodeBox's proxy intercept and forward a specific application's API requests.
...
When takeover is enabled, CodeBox modifies the app's configuration file to point the API endpoint to the local proxy.
...
When disabling takeover, CodeBox will:
...
Before enabling takeover, CodeBox backs up the original configuration and restores it when disabled.
