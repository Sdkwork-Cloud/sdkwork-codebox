> Migrated from `docs/user-manual/ja/1-getting-started/1.5-settings.md` on 2026-06-24.
> Owner: SDKWork maintainers

このセクションでは、個人の好みに合わせて CodeBox を設定する方法を説明します。
...
有効にすると、システム起動時に CodeBox が自動的に起動します。
...
有効にすると、CodeBox はプロバイダー切り替え時に VS Code の Claude Code 拡張に設定を自動同期します（`~/.claude/config.json` の `primaryApiKey` に書き込み）。
...
CodeBox がターミナルを開く際に使用するターミナルアプリケーションを選択します。
...
CodeBox 自体のデータ保存先です。デフォルトは `~/.sdkwork/codebox/` に統一され、Windows では `%USERPROFILE%\\.sdkwork\\codebox\\` に展開されます。
...
CodeBox の送信 HTTP/HTTPS プロキシを設定します。外部 API にプロキシ経由でアクセスする必要がある場合に使用します。
...
| リモートディレクトリ | リモート保存パス（デフォルト `codebox-sync`） |
...
現在の CodeBox バージョン番号を表示し、以下をサポートします：
