> Migrated from `docs/user-manual/en/1-getting-started/1.5-settings.md` on 2026-06-24.
> Owner: SDKWork maintainers

This section describes how to configure CodeBox according to your preferences.
...
CodeBox supports three languages:
...
When enabled, CodeBox automatically runs when the system starts.
...
When enabled, CodeBox automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that CodeBox uses when opening a terminal.
...
The storage location for CodeBox's own data. The default is unified across platforms at `~/.sdkwork/codebox/`. On Windows this resolves to `%USERPROFILE%\\.sdkwork\\codebox\\`.
...
Configure CodeBox's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
| Remote Directory | Remote storage path (default: `codebox-sync`) |
...
Displays the current CodeBox version number, with support for:
