...
fn import_default_config_claude_persists_provider() {
    ...
    assert!(
        db_path.exists(),
        "importing default config should persist to codebox.db"
    );
}
...
fn import_mcp_from_claude_creates_config_and_enables_servers() {
    ...
    assert!(
        db_path.exists(),
        "state.save should persist to codebox.db when changes detected"
    );
}
...
fn set_mcp_enabled_for_codex_writes_live_config() {
    ...
    let toml_path = codebox_lib::get_codex_config_path();
    assert!(
        toml_path.exists(),
