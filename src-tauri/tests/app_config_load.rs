...
use codebox_lib::{AppError, MultiAppConfig};

mod support;
use support::{ensure_test_home, reset_test_fs, test_app_config_dir, test_mutex};

fn cfg_path() -> PathBuf {
    test_app_config_dir().join("config.json")
}
...
fn load_v1_config_returns_error_and_does_not_write() {
    ...
    let bak = test_app_config_dir().join("config.json.bak");
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&codebox_lib::AppType::Claude)
        .is_some());
    assert!(loaded.get_manager(&codebox_lib::AppType::Codex).is_some());
}
