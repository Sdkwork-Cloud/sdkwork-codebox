...
/// 未管理的 Skill（在应用目录中发现但未被 CodeBox 管理）
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl MultiAppConfig {
    ...
    pub fn load() -> Result<Self, AppError> {
        ...
        if is_v1 {
            let default_config_path = crate::config::get_default_app_config_dir().join("config.json");
            let path_text = default_config_path.display().to_string();
    ...
    pub fn save(&self) -> Result<(), AppError> {
        let config_path = get_app_config_path();
        // 先备份旧版（若存在）到当前配置目录下的 config.json.bak，再写入新内容
        if config_path.exists() {
