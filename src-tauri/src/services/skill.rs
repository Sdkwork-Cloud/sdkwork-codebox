...
//!
//! v3.10.0+ 统一管理架构：
//! - SSOT（单一事实源）：`<app-config-dir>/skills/`
...
impl SkillService {
    ...
    /// 获取 SSOT 目录（<app-config-dir>/skills/）
    ...
    /// 扫描未管理的 Skills
    ///
    /// 扫描各应用目录，找出未被 CodeBox 管理的 Skills
    pub fn scan_unmanaged(db: &Arc<Database>) -> Result<Vec<UnmanagedSkill>> {
        ...
        if let Ok(ssot_dir) = Self::get_ssot_dir() {
            scan_sources.push((ssot_dir, "codebox".to_string()));
        }
    ...
    pub fn import_from_apps(
        ...
        search_sources.push((ssot_dir.clone(), "codebox".to_string()));
